...
def originalmarkdown2html(originalhtml, argv=None):
    if argv is None:
        argv = sys.argv
...
import subprocess
import tempfile
styles_file = open("/home/runner/College-Unlocked/styles.html", "r")
styles = styles_file.read()

fonts_file = open("/home/runner/College-Unlocked/fonts.html", "r")
fonts = fonts_file.read()

def markdown_to_html(markdown):
  html = originalmarkdown2html(markdown)
  tmpfile = tempfile.NamedTemporaryFile()
  open(tmpfile.name, 'a+').write(styles+"\n"+html)
  finlist = []
  ok = False
  for x in subprocess.check_output(["/nix/store/bwxril521b35zlf4x2g32hll64rzfhck-nodejs-16.7.0/bin/node", "/home/runner/College-Unlocked/inliner.js", tmpfile.name]).decode("utf-8").strip().split("\n"):
    if x == "</style>":
      ok = True
      continue
    if ok:
